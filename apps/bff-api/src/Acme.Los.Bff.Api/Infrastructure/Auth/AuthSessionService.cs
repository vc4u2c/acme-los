...
public sealed class BffAuthSessionService : IAuthSessionService
{
  ...
  private string? TryReadSessionId(HttpRequest request)
  {
    ...
    return SignedSessionCookie.TryReadSessionId(
      rawCookieValue,
      _environment);
  }
...
internal static class SignedSessionCookie
{
  private const string DevelopmentSessionSecret = "acme-los-local-dev-session-secret";
    ...
    IHostEnvironment environment)
  {
    ...
    try
    {
      var payload = JsonSerializer.Deserialize<SessionCookiePayload>(
  ...
  private sealed record SessionCookiePayload(string SessionId);
}
