    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className="group inline-flex h-9 cursor-pointer appearance-none items-center gap-1.5 rounded-full border border-[var(--border)] bg-[color:var(--surface)/0.92] px-2.5 shadow-[0_10px_22px_var(--shadow-soft)] outline-none transition duration-150 hover:border-[var(--brand)] hover:bg-[var(--surface)] hover:shadow-[0_0_0_1px_var(--brand),0_10px_22px_var(--shadow-soft)] focus:outline-none focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--brand)] focus-visible:ring-offset-1 focus-visible:ring-offset-[var(--background)] sm:h-10 sm:px-3 [-webkit-tap-highlight-color:transparent]"
        >
          <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--surface-strong)] text-[var(--brand)] shadow-sm sm:h-7 sm:w-7">
            <UserRound className="h-[18px] w-[18px]" strokeWidth={2} />
          </span>
          <ChevronDown
            className="h-4 w-4 text-[var(--muted-foreground)] transition-colors group-hover:text-[var(--foreground)]"
            strokeWidth={2}
          />
        </button>
      </DropdownMenuTrigger>
